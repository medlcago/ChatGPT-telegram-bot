...
async def message_to_send(message: types.Message, state: FSMContext, request: Database):
    ...
    if user_exists:
        ...
        await message.reply(
            text=f"user_id <i>{user_id}</i> не найден в базе данных.",
...
async def confirmation_send_message(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await call.message.answer(text=f"Сообщение успешно отправлено пользователю <b>{recipient}</b>",
        ...
        await call.message.edit_text(
            text="Произошла ошибка при отправке сообщения. Возможно, пользователь заблокировал бота.",
            reply_markup=close_button
        )
    finally:
        await state.clear()

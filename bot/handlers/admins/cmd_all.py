...
from bot.keyboards.inline_main import get_confirmation_button, close_button
from bot.states.admins import Administrators

command_all_router = Router()


@command_all_router.message(Command(commands="all"), ChatTypeFilter(is_group=False), IsAdmin())
@MessageLogging
...
@command_all_router.callback_query(F.data.in_({"all", "send_all"}), IsAdmin())
@MessageLogging
async def command_all(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer("Введите сообщение для рассылки")
    await state.set_state(Administrators.Mailing.message)
    await call.answer()


@command_all_router.message(Administrators.Mailing.message)
@MessageLogging
async def message_send_all(message: types.Message, state: FSMContext):
    ...
    await message.copy_to(
        ...
        caption=f"{message.text or message.caption}\n\nПолучатель:\nВсе пользователи бота.",
        reply_markup=markup
    )
...
@command_all_router.callback_query(Administrators.Mailing.confirmation,
...
async def confirmation_send_all(call: types.CallbackQuery, state: FSMContext, request: Database, bot: Bot):
    ...
    sent_message = await call.message.answer(text="Рассылка была запущена.\n\n/cancel - Остановить рассылку",
